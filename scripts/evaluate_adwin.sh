#!/bin/bash
# Script para avaliar as métricas do detector ADWIN
set -e
...
# Prefer sanitized directory (no _full/_tidy). If it doesn't exist, fall back to the
# older directory with the raw dataset name if that exists (backwards compatibility).
